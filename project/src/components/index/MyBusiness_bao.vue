<template>
    <div class="container MyBusinessBao">
        <!-- <div class="main clearfix">
            <div class="videoMain"> 
                <div class="video-nav">
                    <div class="buildItem">
                        我发布的商爆<span class="video-num"> {{totalCount}}个</span>
                    </div>
                    <router-link :to="{ name: 'Publish_bao', params: {}}" class="btnDanger uploadVideo">发布商爆</router-link>
                </div>               
            </div>  
        </div> -->
        <div>
            <div class="centerCon clearfix">
                <div >
                    <template v-if="isNodata_msg" >
                        <NoData textMsg="您还没有发布任何商爆！！！"></NoData>
                    </template>
                  ...
                  <li v-for='item in listData' :key='item.key'>
                     ...
                     <button class='reset'>
                           ...
                           <router-link :to='{name:"Editor_bao",query:{newsId:item.id}}'>修改</router-link>
                          </button>
                      <button style='float:right' class='del' @click='newsAndavenuesDel(item.id)'><img src='../../assets/index/images/icon_删除.png'>删除</button>
...
<script>
    import {newsList,newsAndavenuesDel} from '@/api/api'
    ...
    export default {
        ...
        methods: {
            ...
            //删除
            newsAndavenuesDel(newsId) {
                let data = {
                    newsId :newsId 
                }
                this.$confirm('你确定要删除该条商报吗?', '删除商报', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    newsAndavenuesDel(data).then(res => {
                        if (res.code === 200) {
