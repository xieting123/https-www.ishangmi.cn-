    ...
    <div class="breadcrumb">
      <div class="contain">
        <el-breadcrumb separator-class="el-icon-arrow-right" class="bread">
          ...
          <el-breadcrumb-item :to="{path:'/business_dao'}">商道</el-breadcrumb-item>
          <el-breadcrumb-item :to="{path:'/index/avenuesDetail'}">商道详情</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    </div>
    <div class="containBox">
      <div class="linkTo">
        <ul>
          <li>
            ...
            <div>{{avenuesDetailData.commentCount}}</div>
          </li>
          <li>
          ...
          <li>
            ...
            <div>{{avenuesDetailData.rewardCount}}</div>
          </li>
        </ul>
      </div>
      <div class="linkTo2">
      ...
      <div class="containLeft">
        <span class="titleInfo">
          <span class="origin">{{avenuesDetailData.isOriginal=== 1?'原':'转'}}</span>
          {{avenuesDetailData.newsTitle}}
        </span>

        <div class="desc">
          <router-link
            v-if="userIdentity.identityId===4"
            :to="{path:'/businessmanmemberPersonCenter',query:{userId:avenuesDetailData.trUser.id}}"
          >
            <div
              class="img"
              :style="{backgroundImage:'url(' + avenuesDetailData.trUser.avatar + ')'}"
            ></div>
          </router-link>
          <router-link
            v-else
            :to="{path:'/memberPersonCenterNew',query:{userId:avenuesDetailData.trUser.id}}"
          >
            <div
              class="img"
              :style="{backgroundImage:'url(' + avenuesDetailData.trUser.avatar + ')'}"
        ...
        <div class="sumObtn">
          <button @click="avenuesCommentSubmit">发表</button>
        </div>
      ...
      <div class="ad">
        <div class="rightInfo">
          <div class="card">
            ...
            <img
              class="verifyStatus"
              v-if="avenuesDetailData.trUser.verifyStatus===2"
            ...
            <img
              class="sex"
              v-if="avenuesDetailData.sex==0"
              src="../../assets/index/images/icon_女性.png"
            >
            <img class="sex" v-else src="../../assets/index/images/icon_男性.png">
            <img v-if="avenuesDetailData.trUser.verifyStatus===2" style='width:60px;height:20px;vertical-align: middle;' src='../../assets/index/images/icon_实人认证@3x.png'>
            <!-- <span class="identity">{{userIdentity.identityName}}</span> -->
            <br>
            <p
              class="companyInfo"
            >{{avenuesDetailData.trUser.company}} | {{avenuesDetailData.trUser.position}}</p>
            <!-- <span class='position common'>{{avenuesDetailData.cityName}}</span> -->
        
            <div class="obtns">
              <button
                class="follow"
                @click="accountsCancelLike(avenuesDetailData.trUser.id)"
                v-if="avenuesDetailData.trUser.likeType===4"
              >互相关注</button>
              <button
                class="follow"
                @click="accountsCancelLike(avenuesDetailData.trUser.id)"
                v-else-if="avenuesDetailData.trUser.likeType===2"
              >已关注</button>
              <button
                v-else-if="avenuesDetailData.trUser.likeType===-1||avenuesDetailData.trUser.likeType===0|| avenuesDetailData.trUser.likeType===3"
                class="follow"
                @click="accountsLike(avenuesDetailData.trUser.id)"
              >关注</button>
            </div>
          </div>
  ...
  methods: {
    ...
    avenuesCommentSubmit() {
      if (this.commentForm.newsCommentContent) {
        ...
        avenuesCommentSubmit(data).then(
          res => {
            if (res.code === 200) {
              this.$message.success("提交成功！");
              this.commentForm.newsCommentContent = "";
              this.avenuesCommentList(1);
            } else if (res.code === 10003) {
              this.$router.push({
                path: "/Login",
                query: {
    ...
    //评论列表
    avenuesCommentList(pageNum) {
      let data = {
        newsId: this.newsId,
        pageNum: pageNum,
        pageSize: 5
      };
      avenuesCommentList(data).then(
        res => {
          if (res.code === 200) {
