    ...
    <div class="breadcrumb">
      <div class="contain">
        <el-breadcrumb separator-class="el-icon-arrow-right" class="bread">
          ...
          <el-breadcrumb-item :to="{ name: 'Business_lian', query: {identityId:4}}">商恋</el-breadcrumb-item>
          <el-breadcrumb-item :to="{path:'/businessLianMemberPersonCenter',query:{userId:memberProfile.id}}">商恋详情</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    </div>
    <div class="container">
      <div class="MemberContain">
        <div class="MemberCard">
          ...
          >
          <br>
          <span class="nikeName">{{memberProfile.nickname}}</span>
          <br>
          <img class="sex" v-if="memberProfile.sex==0" src="../../assets/index/images/icon_女性.png">
          <img class="sex" v-else-if="memberProfile.sex==-1" src="">
          <img class="sex" v-else src="../../assets/index/images/icon_男性.png">
          <img v-if="memberProfile.verifyStatus===2" style='width:60px;height:20px;vertical-align: middle;' src='../../assets/index/images/icon_实人认证@3x.png'>
          <br>
          <span class="ID common">{{memberProfile.age}}岁 | {{memberProfile.provinceName}} | </span>
          <br>
          <div class="obtns">
<button class='follow'>
    <router-link v-if='memberProfile.userIdentity.identityId===4' class="linkTo" :to="{ name: 'businessmanmemberPersonCenter', query: {userId:memberProfile.id}}">TA的主页</router-link>
    <router-link v-else class="linkTo" :to="{ name: 'MemberService', query: {userId:memberProfile.id}}">TA的主页</router-link>
    </button>
          </div>
        ...
        <div class="Show">
          ...
          <span class="introduction">内心独白</span>
          <div class="Describe">{{mateSelection.detail}}</div>
          <div class="info">
            <span class="icon"></span>
            <span class="introduction">个人资料</span>
          </div>
          <div class="cartWay">
              <span v-if='mateSelection.user.provinceName||mateSelection.user.cityName'>工作地: {{mateSelection.user.provinceName}} {{mateSelection.user.cityName}}</span>
                    <span v-if='mateSelection.height'>身高：{{mateSelection.height}}cm</span>
                    <span v-if='mateSelection.weight'>体重：{{mateSelection.weight}}kg</span>
                    <span v-if='mateSelection.monthIncome'>月收入: {{mateSelection.monthIncome}}</span>
                    <span v-if='mateSelection.education'>{{mateSelection.education}}</span>
                    <span v-if='mateSelection.maritalStatus'>{{mateSelection.maritalStatus}}</span>
                    <span v-if='mateSelection.wantChild'>是否想要小孩：{{mateSelection.wantChild}}</span>
                    <span v-if='mateSelection.haveChild'>{{mateSelection.haveChild}}</span>
                    <span v-if='mateSelection.smoke'>{{mateSelection.smoke}}</span>
                    <span v-if='mateSelection.drink'>{{mateSelection.drink}}</span>
          
          </div>
          <div class="workExperience">
            <div class="experience">
              <span class="icon"></span>
              <span class="introduction">兴趣爱好</span>
            </div>
            <div class="work">
                <ul>
                    <li class="test2">喜欢的活动: {{mateSelection.activity?mateSelection.activity:'未填写'}}</li>
                    <li class="test2">喜欢的食物: {{mateSelection.food?mateSelection.food:'未填写'}}</li>
                    <li class="test2">喜欢的体育运动: {{mateSelection.sports?mateSelection.sports:'未填写'}}</li>
                    <li class="test2">喜欢的地方: {{mateSelection.place?mateSelection.place:'未填写'}}</li>
                    <li class="test2">喜欢的音乐: {{mateSelection.music?mateSelection.music:'未填写'}}</li>
                    <li class="test2">喜欢的宠物: {{mateSelection.pet?mateSelection.pet:'未填写'}}</li>
                    <li class="test2">喜欢的影视节目: {{mateSelection.video?mateSelection.video:'未填写'}}
                    </li>
                </ul>
          ...
          <div class="eduExperience">
            <div class="experience">
              <span class="icon"></span>
              <span class="introduction">择偶条件</span>
            </div>
            <div class="cartWay2">
                     <span v-if='mateSelection.provinceName||mateSelection.cityName'>工作地: {{mateSelection.provinceName}} {{mateSelection.cityName}}</span>
                    <span v-if='mateSelection.mateHeight'>身高：{{mateSelection.mateHeight}}</span>
                    <span v-if='mateSelection.mateWeight'>体重：{{mateSelection.mateWeight}}</span>
                    <span v-if='mateSelection.mateAge'>年龄: {{mateSelection.mateAge}}</span>
                    <span v-if='mateSelection.mateMonthIncome'>月收入: {{mateSelection.mateMonthIncome}}</span>
                    <span v-if='mateSelection.mateEduction'>{{mateSelection.mateEduction}}</span>
                    <span v-if='mateSelection.mateMaritalStatus'>{{mateSelection.mateMaritalStatus}}</span>
                    <span v-if='mateSelection.mateWantChild'>是否想要小孩： {{mateSelection.mateWantChild}}</span>
                    <span v-if='mateSelection.mateHaveChild'>{{mateSelection.mateHaveChild}}</span>
                    <span v-if='mateSelection.mateSmoke'>{{mateSelection.mateSmoke}}</span>
                    <span v-if='mateSelection.mateDrink'>{{mateSelection.mateDrink}}</span>
            </div>
          </div>
      ...
      viewMemberProfile(data).then(
        res => {
          if (res.code === 200) {
            ...
            //移动端分享
            this.shareTempData.title = this.memberProfile.nickname;
