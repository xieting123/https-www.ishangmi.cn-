...
<span class='introduction'>Ta的粉丝({{OtherFanspagination.totalCount}})</span>  
 ...
 <div class='img' :style="{backgroundImage:'url(' + item.avatar + ')'}" ></div>
           ...
           <span v-if='item.identityId===3||item.identityId===4||item.identityId===6'>
             <span class='name' v-if='item.company===undefined&&item.position===undefined'>{{item.name?item.name:item.nickname}}
                 <img class='ProfileVerifyStatus' v-if='item.verifyStatus===2' src="../../assets/index/images/icon_会员@3x.png" alt="">
             </span>
        <span v-else>
        <span class='nickname'>{{item.name?item.name:item.nickname}}
            <img class='ProfileVerifyStatus' v-if='item.verifyStatus===2' src="../../assets/index/images/icon_会员@3x.png" alt="">
        </span>
        <span class='position'>{{item.company}}.{{item.position}}</span>
         ...
         <span v-else>
             <span class='name' v-if='item.company===undefined&&item.position===undefined'>{{item.nickname}}
                 <img class='ProfileVerifyStatus' v-if='item.verifyStatus===2' src="../../assets/index/images/icon_会员@3x.png" alt="">
             </span>
        <span v-else>
        <span class='nickname'>{{item.nickname}}
            <img class='ProfileVerifyStatus' v-if='item.verifyStatus===2' src="../../assets/index/images/icon_会员@3x.png" alt="">
        </span>
        <span class='position'>{{item.company}}.{{item.position}}</span>
        </span>
        </span>
           <button v-if='item.type===3 || item.type===0 || item.type===-1' @click='accountsLike(item.friendId)' :key='item.id'>关注</button>
        <button v-else-if='item.type===4' :key='item.id' @click='accountsCancelLike(item.friendId)'>互相关注</button>
         <button v-else-if='item.type===2' :key='item.id' @click='accountsCancelLike(item.friendId)'>已关注</button>
  ...
  <div class="pagination paginationIndex">
                    <!-- 分页  -->
                        <el-pagination class="paginationWrap_nav"
                            ...
                            :total="fansListTotal"
                            :current-page=1
                            :page-size='fansListPageSize'
                            v-if="fansListTotal>fansListPageSize"
                            >
                        </el-pagination>
                    </div>
</div>
...
<script>
    ...
    import {getListLim,
        ...
        getFansList,getOtherFriendsList,getOtherFansList
        // mobileShare
    } from '../../api/api'
        ...
        data(){
            return {
             ...
             OtherFanspagination:[],
                ...
                business_ju: [],
                business_pin: [],
                fansListTotal:0,//他的粉丝
                fansListPageSize:0,//他的粉丝分页
                totalCount:0,//商脉
                totalCount1:0,//商机
                totalCount2:0,//商会
                totalCount3:0,//商询
       ...
       handleClick(tab, event) {
            ...
            getOtherFriendsList(pageNum,pageSize){
                 var data = {
                    userId :this.userId ,
                    pageNum:pageNum,
                    pageSize:this.OtherFanspagination.totalCount
                }
                getOtherFriendsList(data).then(res=>{
                    if(res.code===200){  
           ...
           getOtherFansList(pageNum,pageSize){
                ...
                getOtherFansList(data).then(res=>{
                    if(res.code===200){     
                this.OtherFansList=res.result.list;
                this.OtherFanspagination=res.result.pagination;
                 this.fansListTotal=res.result.pagination.totalCount;
                this.fansListPageSize=res.result.pagination.pageSize
                    }
                    else {
                        this.$message({
                            message: res.msg,
