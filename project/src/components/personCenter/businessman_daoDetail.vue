<template>
  <div class="memberPersonCenterNew">
      ...
      <div class="contain">
        <el-breadcrumb separator-class="el-icon-arrow-right" class="bread">
          ...
          <el-breadcrumb-item :to="{path:'/businessmanmemberPersonCenter',query:{userId:memberProfile.id}}">商人详情</el-breadcrumb-item>
           <el-breadcrumb-item :to="{path:'/businessmanDaoDetail',query:{userId:memberProfile.id}}">{{memberProfile.name}}的商道</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    </div>
    <div class="container">
      <div class="MemberContain">
        <div class="MemberCard">
          <router-link v-if='memberProfile.userIdentity.identityId===4' :to="{path:'/businessmanmemberPersonCenter',query:{userId:memberProfile.id}}">
      <div class="avatar" :style="{backgroundImage:'url(' + memberProfile.avatar + ')'}"></div>
        ...
        <div class="shangdaoList"  v-if='business_dao.length!==0'>
        <ul style="min-height: 300px;" class="clearfix" v-loading="loading">
          <li v-for="item in business_dao" v-bind:key="item.key">
    ...
    <div class="oImg" v-if="item.imageArr.length>0"  :style="{backgroundImage:'url('+item.imageArr[0]+')'  }"></div>
            <div class="shangbaoBox">
              ...
              <span class="info"><span class="orige">{{item.isOriginal==1?'原':'转'}}</span>{{item.newsTitle}}</span>
  ...
  <div class="pagination paginationIndex">
        ...
        <div class="Show" v-else>暂无商道！！！</div>
      </div>
    </div>
