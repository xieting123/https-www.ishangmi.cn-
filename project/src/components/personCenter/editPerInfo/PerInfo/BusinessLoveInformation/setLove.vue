...
<script>
 import {updateMateInformation,queryTrOptionsAll,getMateSelectionByUserId,updateMateStatus,myProfileByToken} from '@/api/api';
    export default {
        data(){
            return {
                isMate:false,
        ...
        methods:{
            ...
            updateMateStatus(){
                ...
                let data = {
                    userId:userId,
                    isMate:this.isMate?1:0
                }
                updateMateStatus(data).then(res=>{
                    if(res.code===200){
                       if(this.isMate){
                           alert('开启商恋')
                       }else{
            ...
            //获取自己的个人信息
                ...
                myProfileByToken().then(
                 res => {
                if (res.code === 200) {
              this.isMate=res.result.isMate===1?true:false;
          }
